...
-- Create organizer_connect table if it doesn't exist (using payout_accounts as base)
-- This maps to the existing payout_accounts table
...
CREATE POLICY IF NOT EXISTS "organizers_manage_packages" ON public.sponsorship_packages
  FOR ALL USING (
    EXISTS (
...
CREATE POLICY IF NOT EXISTS "sponsors_read_packages" ON public.sponsorship_packages
  FOR SELECT USING (visibility = 'public' AND is_active = true);

-- Update existing RLS policies for sponsorship_orders to include new fields
