-- Fix ad_spend_ledger to include wallet_id from campaign
...
BEGIN
  ...
  IF p_pricing_model = 'cpm' AND p_rate_credits > 0 THEN
    ...
    IF v_is_viewable THEN
      -- Get wallet_id from campaign's organization
      ...
      v_whole := FLOOR(v_acc);
      IF v_whole >= 1 THEN
        -- Log whole credits to spend ledger (using org_wallet_id)
        ...
        VALUES (
          ...
          1,
          'cpm',
          FLOOR(p_rate_credits * 100)::INTEGER, -- Convert credits to USD cents (1 credit = $1)
          v_whole,
          NOW()
        );
...
BEGIN
  ...
  IF p_pricing_model = 'cpc' AND p_bid_credits > 0 THEN
    ...
    -- Log to spend ledger (using org_wallet_id)
    INSERT INTO campaigns.ad_spend_ledger (
      campaign_id,
      org_wallet_id,
      creative_id,
    ...
    VALUES (
      ...
      1,
      'cpc',
      FLOOR(p_bid_credits * 100)::INTEGER, -- Convert credits to USD cents (1 credit = $1)
      p_bid_credits,
      NOW()
    );
