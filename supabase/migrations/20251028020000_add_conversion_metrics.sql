...
-- =====================================================
-- Adds computed columns for CTR, CVR, ROAS, CPA, and
-- view-through attribution insights
-- =====================================================

-- 1. Drop and recreate the materialized view with new metrics
...
WITH daily_stats AS (
  SELECT
    ...
    COALESCE(spend.spend_credits, 0) + COALESCE(c.spend_accrual, 0) AS spend_credits,
...
COMMENT ON MATERIALIZED VIEW public.analytics_campaign_daily_mv IS 
  'Campaign analytics with computed metrics (CTR, CVR, ROAS, CPA, viewability). Refresh with refresh_analytics().';

COMMENT ON COLUMN public.analytics_campaign_daily_mv.ctr IS 'Click-through rate: (clicks / impressions) * 100';
COMMENT ON COLUMN public.analytics_campaign_daily_mv.cvr IS 'Conversion rate: (conversions / clicks) * 100';
COMMENT ON COLUMN public.analytics_campaign_daily_mv.cpm IS 'Cost per mille: (spend / impressions) * 1000';
COMMENT ON COLUMN public.analytics_campaign_daily_mv.cpc IS 'Cost per click: spend / clicks';
COMMENT ON COLUMN public.analytics_campaign_daily_mv.cpa IS 'Cost per acquisition: spend / conversions';
COMMENT ON COLUMN public.analytics_campaign_daily_mv.roas IS 'Return on ad spend: revenue / spend';
COMMENT ON COLUMN public.analytics_campaign_daily_mv.viewability_rate IS 'Viewable impressions / total impressions * 100';
COMMENT ON COLUMN public.analytics_campaign_daily_mv.view_through_rate IS 'View-through conversions / total conversions * 100';

-- 2. Update the refresh function to include new metrics
...
GRANT EXECUTE ON FUNCTION public.refresh_analytics() TO authenticated, service_role;

COMMENT ON FUNCTION public.refresh_analytics IS 
  'Refreshes all analytics materialized views with computed conversion metrics';
