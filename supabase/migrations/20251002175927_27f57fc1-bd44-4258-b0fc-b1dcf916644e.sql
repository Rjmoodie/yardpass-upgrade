-- Drop ALL old versions to avoid signature conflicts
DROP FUNCTION IF EXISTS public.rpc_campaign_analytics_daily(uuid, date, date, uuid[]);
DROP FUNCTION IF EXISTS public.rpc_creative_analytics_rollup(uuid, text, text, uuid[], uuid[], boolean, text, text, int, int);
DROP FUNCTION IF EXISTS public.rpc_creative_analytics_rollup(uuid, date, date, uuid[], uuid[], boolean, text, text, int, int);

-- Create hardened campaign analytics daily RPC
...
-- Create hardened creative analytics rollup RPC
...
CREATE INDEX IF NOT EXISTS idx_crad_org_date ON creative_analytics_daily(org_id, date);
CREATE INDEX IF NOT EXISTS idx_ad_creatives_campaign ON ad_creatives(campaign_id, id);