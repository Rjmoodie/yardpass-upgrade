-- Drop ALL versions of search_all function to resolve overloading conflict
DROP FUNCTION IF EXISTS public.search_all(uuid,text,text,date,date,boolean,integer,integer);
DROP FUNCTION IF EXISTS public.search_all(uuid,text,text,timestamp with time zone,timestamp with time zone,boolean,integer,integer);

-- Create the final version with clear parameter types
