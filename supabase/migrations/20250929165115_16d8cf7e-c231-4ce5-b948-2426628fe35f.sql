-- Allow older posts to be searchable even if their events are past
...
-- Events candidate set (only include future events for event discovery)
events_base AS (
  SELECT
  ...
  WHERE
    -- Only include future events for event discovery
    e.start_at >= now()
    AND (
...
-- Posts candidate set (include posts from past events to keep them searchable)
posts_base AS (
  SELECT
  ...
  WHERE
    -- Allow posts from past events to remain searchable
    (
    ...
    -- Category filter based on event
    AND (p_category IS NULL OR e.category = p_category)
    -- Date filters apply to post creation date, not event date
    AND (p_date_from IS NULL OR p.created_at >= (p_date_from::date)::timestamptz)
    AND (p_date_to IS NULL OR p.created_at < ((p_date_to::date + 1))::timestamptz)
