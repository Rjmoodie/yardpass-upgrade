...
create table if not exists public.user_profiles (
  ...
  phone text,
  photo_url text,
  role text default 'attendee', -- ui toggle 'attendee'|'creator'
...
create table if not exists public.events (
  id uuid primary key default gen_random_uuid(),
  owner_context_type owner_context not null,   -- 'individual'|'organization'
  owner_context_id uuid not null,              -- auth.users.id or organizations.id
  created_by uuid not null references auth.users(id),
...
create table if not exists public.ticket_tiers (
  ...
  badge_label text,                      -- displayed badge text
  price_cents int not null default 0,    -- 0 for free
  currency text not null default 'USD',
  quantity int,                          -- null = unlimited
  max_per_order int default 6,
  sales_start timestamptz,
  sales_end timestamptz,
  status text default 'active',          -- 'active'|'hidden'|'archived'
...
create table if not exists public.orders (
  ...
  payout_destination_owner owner_context,     -- snapshot of seller of record
...
create table if not exists public.tickets (
  ...
  owner_user_id uuid not null references auth.users(id), -- changes on transfer
  status ticket_status not null default 'issued',
  qr_code text unique not null,  -- store a signed opaque token
  wallet_pass_url text,          -- Apple/Google
...
create table if not exists public.event_posts (
  ...
  ticket_tier_id uuid references public.ticket_tiers(id), -- for badge at post time
  text text,
  media_urls text[] default '{}', -- short-form videos/images
  created_at timestamptz default now(),
...
create table if not exists public.payout_accounts (
  id uuid primary key default gen_random_uuid(),
  context_type owner_context not null,         -- individual | organization
  context_id uuid not null,                    -- auth.users.id or organizations.id
...
```sql
-- Highest-tier badge for a user within an event
...
-- Eligibility to post: ticket holder OR event manager
...
-- Orgs & Memberships
create policy if not exists orgs_read_all
  on public.organizations for select using (true);

create policy if not exists orgs_insert_auth
  on public.organizations for insert with check (auth.role() = 'authenticated');

create policy if not exists orgs_update_admin
...
create policy if not exists org_memberships_read_self
  on public.org_memberships for select
  using (user_id = auth.uid() or public.is_org_role(org_id, array['admin','owner']));

create policy if not exists org_memberships_write_admin
...
-- Events
create policy if not exists events_read_public_or_manager
...
create policy if not exists events_insert_owner_or_editor
...
create policy if not exists tiers_write_manager
...
create policy if not exists orders_select_owner_or_manager
  on public.orders for select
  using (user_id = auth.uid() or public.is_event_manager(event_id));

create policy if not exists order_items_select_owner_or_manager
...
create policy if not exists tickets_select_owner_or_manager
  on public.tickets for select
  using (owner_user_id = auth.uid() or public.is_event_manager(event_id));

create policy if not exists refunds_select_owner_or_manager
  on public.refunds for select
  using (exists (select 1 from public.orders o where o.id = order_id and (o.user_id = auth.uid() or public.is_event_manager(o.event_id))));

-- Posts (ticket-gated creation)
create policy if not exists posts_read_public
  on public.event_posts for select
  using (exists (select 1 from public.events e where e.id = event_id and (e.visibility='public' or public.is_event_manager(e.id))));

create policy if not exists posts_insert_ticket_or_org
...
create policy if not exists posts_delete_author_or_admin
...
create policy if not exists comments_insert_any_auth
  on public.event_comments for insert
  with check (auth.role() = 'authenticated');

create policy if not exists comments_update_author_or_admin
...
create policy if not exists comments_delete_author_or_admin
...
create policy if not exists payout_accounts_select_self
...
```sql
-- Prevent deleting tiers with issued tickets
...
-- Prevent price changes once a tier has sold at least one ticket
...
- Writes to `orders`, `order_items`, `tickets`, `refunds`, and `payout_accounts` should be done **via Edge Functions with the service role** (bypassing RLS).  
- Posting eligibility is **enforced at DB level** with `posts_insert_ticket_or_org` policy + `can_current_user_post()` helper.  
- For performance on large events, consider a **cache table** `user_event_badges` that mirrors `user_event_badge_v` on ticket issue/transfer/redeem webhooks.
