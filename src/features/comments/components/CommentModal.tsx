  ...
  return (
    <Dialog key={dialogKey} open={isOpen} onOpenChange={onClose}>
      <DialogContent className="w-[calc(100vw-2rem)] max-w-[1200px] h-[90vh] max-h-[90vh] p-0 gap-0 overflow-hidden bg-background border-2 border-border dark:border-white/20 shadow-[0_32px_96px_-16px_rgba(0,0,0,0.5)] ring-1 ring-black/10 dark:ring-white/10 rounded-2xl flex flex-col">
        {/* Header */}
        <DialogHeader className="sticky top-0 z-10 bg-background/95 backdrop-blur-md border-b border-border/80 px-3 sm:px-5 py-2.5 sm:py-3.5">
          <div className="flex items-center justify-between gap-3">
            <DialogTitle className="flex min-w-0 flex-1 flex-col gap-1">
              {/* Top row: icon pill + main label */}
              <div className="flex items-center gap-2 min-w-0">
                ...
                <div className="flex flex-col min-w-0">
                  ...
                  <div className="flex items-center gap-1.5 text-[11px] sm:text-xs text-muted-foreground min-w-0">
                    {activePost ? (
                      <span>
                        {activePost.comment_count ?? topLevelComments.length} comment
                        {(activePost.comment_count ?? topLevelComments.length) === 1 ? '' : 's'}
                      </span>
                    ) : (
            ...
            <div className="flex items-center gap-1.5 shrink-0">
              {!singleMode && posts.length > 1 && activePost && (
                <div className="hidden sm:flex items-center gap-2">
                  <span className="text-[11px] text-muted-foreground">
                    Post {posts.indexOf(activePost) + 1} / {posts.length}
                  </span>
                  <div className="relative">
                    <select
                      className="text-xs bg-muted/40 hover:bg-muted/60 border border-border rounded-lg px-2.5 pr-7 py-1.5 cursor-pointer transition-colors"
                      value={activePost.id}
                    ...
                    >
                      {posts.map((p, idx) => (
                        <option key={p.id} value={p.id} title={p.text}>
                          Post {idx + 1}: {p.text?.slice(0, 30) || 'View'}
                          {p.text && p.text.length > 30 ? '…' : ''}
                        </option>
                      ))}
                    </select>
                    <ChevronDown className="w-3 h-3 absolute right-2 top-1/2 -translate-y-1/2 pointer-events-none text-foreground/65" />
                  </div>
                </div>
              )}
