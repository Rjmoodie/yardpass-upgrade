...
interface OrgBuyCreditsModalProps {
  open: boolean;
  onOpenChange: (open: boolean) => void;
  packages: CreditPackage[];
  onPurchase: (args: { package_id?: string; custom_credits?: number; promo_code?: string }) => Promise<any>;
  isPurchasing: boolean;
  isLoadingPackages?: boolean;
}

export const OrgBuyCreditsModal = ({
  open,
  onOpenChange,
  packages,
  onPurchase,
  ...
  const selectedAmountCents = useMemo(() => {
    if (selectedPackage === "custom") {
      ...
      return Number.isFinite(credits) && credits >= 1000 ? credits : 0;
    }
  ...
  const handlePurchase = async () => {
    try {
      if (selectedPackage === "custom") {
        ...
        await onPurchase({ custom_credits: credits, promo_code: promoCode || undefined });
      } else if (selectedPackage) {
        await onPurchase({ package_id: selectedPackage, promo_code: promoCode || undefined });
      }
      toast({
        title: "Purchase started",
        description: "We'll update your balance once payment completes.",
      });
      onOpenChange(false);
    } catch (e: any) {
      toast({
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent
        className="max-w-md"
        aria-description="Choose a credit package and complete payment"
      >
        <DialogHeader>
                    ...
                    <div className="flex justify-between items-center">
                      ...
                      <span className="font-medium">
                        {new Intl.NumberFormat(undefined, {
                          style: "currency",
                          currency: "USD",
                        }).format(pkg.price_usd_cents / 100)}
                      </span>
                    </div>
                  </Label>
                </div>
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-sm text-muted-foreground">
                      ...
                      {new Intl.NumberFormat(undefined, {
                        style: "currency",
                        currency: "USD",
                      }).format((Number(customCredits) || 0) / 100)}
                    </span>
                  </div>
                </Label>
              </div>
