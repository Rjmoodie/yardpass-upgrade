-- Create a fresh test event for k6 load testing (using existing organization)
...
DECLARE
    ...
    v_tier_id UUID;
BEGIN
    -- Find an existing organization or create one with a real user
    SELECT id INTO v_org_id FROM organizations LIMIT 1;
    
    IF v_org_id IS NULL THEN
        -- If no organizations exist, we need to create one with a real user
