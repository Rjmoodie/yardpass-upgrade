-- Create a fresh test event for k6 load testing (simplified approach)
...
BEGIN
    ...
    -- Find a real user from user_profiles (not auth.users)
    SELECT user_id INTO v_user_id FROM user_profiles LIMIT 1;
    
    IF v_user_id IS NULL THEN
        RAISE EXCEPTION 'No user profiles found. Please ensure you have user profiles.';
    END IF;
    
    -- Create the test event with proper created_by
    INSERT INTO public.events (
        ...
        now() + interval '30 days' + interval '4 hours', -- 4 hour event
        'Test Venue',
        'Test City',
        '123 Test St',
